...
class Config():
    ...
    """
    def __init__(self, config_path: str=None, file_path: str=None, **kwargs):
        """
    ...
    def set_random_seed(self):
        random.seed(self.seed_init)
        np.random.seed(self.seed_init)
        torch.manual_seed(self.seed_init)
        torch.backends.cudnn.benchmark = False
        torch.backends.cudnn.deterministic = True
        if is_available():
            torch.cuda.manual_seed(self.seed_init)
            ...
            if self.cuda_index == -1:
                ...
                for i in range(gpu_count):
                    ...
                    if memuse < min_memuse:
                        min_memuse = memuse
                        self.cuda_index = i
            elif self.cuda_index < gpu_count:
