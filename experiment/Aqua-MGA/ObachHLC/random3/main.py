...
from general_code.train.runNN import EarlyStopping
    ...
    for time_id in range(config.eval_times):
        ...
        for epoch in range(config.num_epochs):
            train_OOD_epoch(device=config.device, 
                        ...
                        metric=config.metric,
                        logger=logger)
            val_result = eval_OOD_epoch(
                device=config.device, 
                models=models, 
                data_loader=val_loader, 
                metric=config.metric, 
                loss_criterion=config.loss_criterion)[0]
        ...
        test_result = eval_OOD_epoch(config.device, models, test_loader, config.metric, extra_metrics, loss_criterion=config.loss_criterion)
        train_result = eval_OOD_epoch(config.device, models, train_loader, config.metric, extra_metrics, loss_criterion=config.loss_criterion)
        val_result = eval_OOD_epoch(config.device, models, val_loader, config.metric, extra_metrics, loss_criterion=config.loss_criterion)
